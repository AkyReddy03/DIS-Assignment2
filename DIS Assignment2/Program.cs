    ...
    class Program
    {
        ...
        */
        ...
        public static IList<IList<int>> FindMissingRanges(int[] nums, int lower, int upper)
        {
            try
            {
                ...
                if (lower < nums[0])
                {
                    // Add a range to the missingRanges list
        ...
        */
        ...
        public static int MaxProfit(int[] prices)
        {
            try
            {
                int minPrice = int.MaxValue;  // Initialize the minimum price
        ...
        */
        ...
        public static bool IsStrobogrammatic(string s)
        {
            try
            {
                string strobogrammaticDigits = "01689";  // Define strobogrammatic digits
                int left = 0;
                int right = s.Length - 1;

                while (left <= right)
                {
                    if (!strobogrammaticDigits.Contains(s[left].ToString()) ||
                        !strobogrammaticDigits.Contains(s[right].ToString()))
                    {
                        return false;  // Non-strobogrammatic digit found
                    }

                    // Check if the pair of digits is strobogrammatic
                    if (s[left] == s[right] ||                 // 0, 1, 8
                        (s[left] == '6' && s[right] == '9') ||  // 6 and 9
                        (s[left] == '9' && s[right] == '6'))
                    {
                        left++;
                        right--;
                    ...
                    {
                        return false;  // Non-strobogrammatic pair
                    }
                }
        ...
        */
        ...
        public static int ThirdMax(int[] nums)
        {
            try
            {
                long firstMax = long.MinValue;  // Initialize the first maximum value
                long secondMax = long.MinValue; // Initialize the second maximum value
                long thirdMax = long.MinValue;  // Initialize the third maximum value
                ...
                foreach (int num in nums)
                {
                    if (num > firstMax)
                    {
                        thirdMax = secondMax; // Shift values to find the third maximum
                        secondMax = firstMax; // Shift values to find the second maximum
                        firstMax = num;       // Update the first maximum
                    }
                    else if (num < firstMax && num > secondMax)
                    {
                        thirdMax = secondMax; // Shift values to find the third maximum
                        secondMax = num;      // Update the second maximum
                    }
                    else if (num < secondMax && num > thirdMax)
                    {
                        thirdMax = num;       // Update the third maximum
                    }
                }
                ...
                return (int)thirdMax; // Return the third maximum value
            }
            catch (Exception)
            {
                throw;
